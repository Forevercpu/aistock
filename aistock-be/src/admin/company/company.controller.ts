...
export class AdminCompanyController {
  ...
  updateStatus(@Param('id', ParseIntPipe) id: number, @Body() dto: UpdateCompanyStatusDto) {
    return this.companyService.updateStatus(id, dto);
  }

  @Post(':id/products')
  @ApiOperation({ summary: '新增公司主营产品' })
  createProduct(@Param('id', ParseIntPipe) id: number, @Body() dto: CreateProductDto) {
  ...
  deleteProduct(@Param('id', ParseIntPipe) id: number, @Param('productId', ParseIntPipe) productId: number) {
    return this.companyService.deleteProduct(id, productId);
  }
}
